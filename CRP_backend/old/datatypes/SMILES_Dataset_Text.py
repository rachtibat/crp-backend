...
class SMILES_Dataset:
    ...
    def visualize_heatmap(self, relevance, size, index) -> np.ndarray:
            """
            ...
            # contribs contains contributions of atoms only, this makes it quite complicated to parse
            # the relevance....
            ...
            for i, e in enumerate(sample[0]):
                ...
                c_rgb = np.array(scalarMap.to_rgba(rel_s))[:3] * 255
                #c_rgb = np.array(cm.bwr(rel_s))[:3] * 255

                if letter == " ":
                    break

                (text_width, text_height) = cv2.getTextSize(letter, font, font_size, thick)[0]
                text_height += 15
                mask = np.ones((text_height, text_width, 3), dtype=np.uint8) * 255
                mask = cv2.putText(mask, letter, (0, 15), font,font_size, c_rgb, thick,cv2.LINE_AA)
                all_masks.append(mask)

                acc_text_width += mask.shape[1]

            
            max_v = acc_text_width if acc_text_width > text_height else text_height
            img = np.ones((max_v, max_v, 3), dtype=np.uint8) * 255

            last = 0

            for m in all_masks:
                img[:text_height, last:last+m.shape[1], :] = m
                last += m.shape[1]

            #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            
            heatmap = rescale_image(img, size)

            return heatmap

    def visualize_data_sample(self, ecfp, size: int, data_index):
        ...
        _, _, smile, _ = self.dataset[data_index]
        
        mol = Chem.MolFromSmiles(smile)

        img = Draw.MolToImage(mol).convert("RGB") # bug fixed through convertion
        img = np.array(img)

        return rescale_image(img, size)
    ...
    def gen_visualize_filter(self, codes, most_neuron_index, rf, rf_to_neuron_index, filter_index, size, selected=(0, 10),
        ...
        for i_c, cod in enumerate(codes):
          ...
          #  fragsmi = AllChem.MolFragmentToSmiles(mol, atom_nr)
           # mol = Chem.MolFromSmiles(fragsmi)
        ...
        #    else:
          #      atom_nr = []
            ...
            label = sg.TextElement(0, 30, smiles_in_rf, size=30, 
                                font='sans-serif')
            fig.append(label)
            svg = fig.to_str()
    ...
    def gen_visualize_heatmap_filter(self, heatmaps, n_indices, rf, rf_to_neuron_index, filter_index, size,
                                padding, data_indices):


        images_list = []
        for i in range(len(heatmaps)):
            heat = heatmaps[i] / (abs(heatmaps[i]).max() + 1e-10)
            images_list.append(self.visualize_heatmap(heat, size, data_indices[i]))
