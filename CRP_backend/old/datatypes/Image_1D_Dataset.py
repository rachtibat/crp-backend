...
from typing import List, Iterable, Tuple, Any, Union, Iterator, Dict
import numpy as np
...
mpl.rcParams['axes.linewidth'] = 1.5  # set border width


class Image_1D_Dataset:
    ...
    def decode_pred(self, pred: torch.tensor, N: int) -> List[str]:
        """
    ...
    def convert_to_2D_plot(self, series_np, heatmap_np=None):
       ...
       # ax.set_ylabel("Normalized Amplitude", fontsize=12)
        ...
        if heatmap_np is not None:
            ...
            ax.plot(series_np, alpha=0.1)

            x = np.arange(0, len(series_np))
            points = np.array([x, series_np]).T.reshape(-1, 1, 2)
            segments = np.concatenate([points[:-1], points[1:]], axis=1)
            norm = plt.Normalize(vmin=-1, vmax=1)
            lc = LineCollection(segments, cmap='bwr', norm=norm)

            lc.set_array(heatmap_np)
            lc.set_linewidth(2)
            _ = ax.add_collection(lc)

        else:

            ax.plot(series_np, '-', color="black", linewidth=2, alpha=1)
        ...
        im_array = data.reshape((int(h), int(w), -1))

        plt.close(fig)

        return im_array

    def visualize_data_sample(self, series_np: np.ndarray, size: int, padding=False) -> np.ndarray:
        """
        ...
        series_np = series_np[0]  # remove channel dimension
        #series_np = np.ones((50, 50), dtype=np.uint8)
        series_np = self.convert_to_2D_plot(series_np)
        rescaled = rescale_image(series_np, size)

        return rescaled

    def visualize_heatmap(self, relevance, size, sample_index) -> np.ndarray:
        """
        ...
        series_tensor, _ = self.get_data_sample(sample_index)

        # remove batch and channel dimension
        series_np = series_tensor.detach().cpu().numpy()[0, 0]
        heatmap = self.convert_to_2D_plot(series_np, relevance)
    ...
    def init_relevance_of_target(self, targets, pred) -> Iterable[torch.tensor]:
        ...
        Args:
            target: batch/list of several self.get_data_sample[1]
        ...
        if type(targets[0]) == int:
            ...
            r[batch_indices, targets] = 1
            return [r]
        elif targets.shape == pred.shape:
            return [targets]
        else:
            raise ValueError("Error in <init_relevance_of_target>")

    def generate_input_attr_mask(self, inp_tensor, x, y, width, height):
        """
        ...
        inp_len = inp_tensor.shape[-1]
        x = int(x * inp_len)
        width = int(width * inp_len)

        mask = np.zeros(inp_len)
        mask[x:x + width] = 1
    ...
    def extract_rel_in_region(self, inp_attr_ch: np.ndarray, mask: np.ndarray):
        ...
        return np.sum(rel_in_mask, axis=1)

    def gen_visualize_filter(self, tensor_series, most_neuron_index, rf, rf_to_neuron_index, filter_index, size,
        ...
        cropped_img = self.mask_images_with_rf(rf_n, tensor_series)

        visualized_img = []
        for im in cropped_img:
            visualized_img.append(
                self.visualize_data_sample(im, size, padding))
    ...
    def gen_visualize_heatmap_filter(self, heatmaps_np, neuron_indices, rf, rf_to_neuron_index, filter_index, size,
                                     padding=False, data_indices=[]):

        series_tensor, _ = self.get_data_concurrently(data_indices)

        series_np = series_tensor.detach().cpu().numpy()

        rf_indices = [rf_to_neuron_index[index] for index in neuron_indices]
        rf_n = rf[rf_indices]
        rf_n = torch.from_numpy(rf_n).to(self.device)

        heats, limits = self.mask_heatmap_with_rf(rf_n, heatmaps_np)

        images_list = []
        for i in range(len(heats)):
            l1, l2 = limits[i]
            heats[i] = norm_attribution(heats[i], batch_dim=False)
            image = self.convert_to_2D_plot(series_np[i, 0, l1:l2], heats[i])
            rescaled = rescale_image(image, size)
            images_list.append(rescaled)

        return images_list

    def mask_images_with_rf(self, rf: torch.Tensor, series: torch.Tensor, threshold=40) -> list:
        """
        functions multiplies the receptive field with the input image.
        Moreover, the region outside of the receptive field (elements with zero value) are cut out.
        ...
        for i in range(len(rf)):
            rows = torch.where(rf[i] > threshold)[0]
            row1 = rows.min() if len(rows) != 0 else 0
            row2 = rows.max() if len(rows) != 0 else -1

            if (row1 < row2):
                cropped = series[i, :, row1:row2].detach().cpu().numpy()
            else:
                cropped = series[i].detach().cpu().numpy()

            cropped_s.append(cropped)

        return cropped_s

    def mask_heatmap_with_rf(self, rf: torch.Tensor, heatmap_np, threshold=40) -> list:
        """
        functions multiplies the receptive field with the input image.
        Moreover, the region outside of the receptive field (elements with zero value) are cut out.
        ...
        for i in range(len(rf)):
            ...
            if (row1 < row2):
                cropped = heatmap_np[i, row1:row2]
                limits.append((row1.item(), row2.item()))
            else:
                cropped = heatmap_np[i]
                limits.append((0, len(cropped)))

            cropped_s.append(cropped)

        return cropped_s, limits
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for index in indices:
                future = executor.submit(
                    self.get_data_sample, index, preprocessing)
